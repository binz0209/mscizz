...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    def __str__(self):
        return '**{0.title}** đăng bởi **{0.uploader}**'.format(self)
    ...
    async def create_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        ...
        if data is None:
            raise YTDLError('Không thể tìm thấy `{}`'.format(search))
        ...
        if processed_info is None:
            raise YTDLError('Không thể phát `{}`'.format(webpage_url))
        ...
        if 'entries' not in processed_info:
            ...
            while info is None:
                try:
                    ...
                    raise YTDLError('Không có kết quả phù hợp nào cho `{}`'.format(webpage_url))
...
class Song:
    ...
    def create_embed(self):
        embed = (discord.Embed(title='Đang phát',
                               description='***\n{0.source.title}\n\n***'.format(self),
                               color=discord.Colour(0xff33ff))
                 .add_field(name='Thời gian', value=self.source.duration)
                 .add_field(name='Người yêu cầu', value=self.requester.mention)
                 .add_field(name='Người đăng', value='[{0.source.uploader}]({0.source.uploader_url})'.format(self))
                 .add_field(name='URL', value='[Click]({0.source.url})'.format(self))
                 .add_field(name='Made by', value='Binz (Bé Tư Linh)')
...
class Music(commands.Cog):
    ...
    def cog_check(self, ctx: commands.Context):
        if not ctx.guild:
            raise commands.NoPrivateMessage('Lệnh không được sử dụng trong tin nhắn riêng')
    ...
    async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
        ...
        if not channel and not ctx.author.voice:
            raise VoiceError('Bạn chưa kết nối với kênh thoại cũng như không được chỉ định kênh để tham gia.')
    ...
    async def _leave(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.voice:
            return await ctx.send('Không có kết nối với kênh thoại.')
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('Không có bài nào đang được phát.')

        if 0 > volume > 100:
            return await ctx.send('Âm lượng chỉnh từ 0 đến 100')

        ctx.voice_state.volume = volume / 100
        await ctx.send('Âm lượng được chỉnh đến {}%'.format(volume))
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            if total_votes >= 3:
                ...
                await ctx.send('Bỏ qua phiếu bầu đã thêm, hiện tại **{}/3**'.format(total_votes))

        else:
            await ctx.send('Bạn đã bình chọn để bỏ qua bài hát này.')
    ...
    async def _queue(self, ctx: commands.Context, *, page: int = 1):
        ...
        embed = (discord.Embed(description='**{} Bài hát:**\n\n{}'.format(len(ctx.voice_state.songs), queue))
                 .set_footer(text='Đang xem trang {}/{}'.format(page, pages)))
    ...
    async def _loop(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('Không có gì được phát vào lúc này.')
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.send('Đã xảy ra lỗi khi xử lí yêu cầu này: {}'.format(str(e)))
            else:
                song = Song(source)

                await ctx.voice_state.songs.put(song)
                await ctx.send('Đã thêm {}'.format(str(source)))

    @_join.before_invoke
    @_play.before_invoke
    async def ensure_voice_state(self, ctx: commands.Context):
        if not ctx.author.voice or not ctx.author.voice.channel:
            raise commands.CommandError('Bạn không có ở trong một kênh thoại nào.')
